...
def _resolve_checkpoint(client, path: str, csrf: str) -> bool:
    ...
    if not choice:
        ...
        if body:
            ...
            _say("[!]", "")
            _say("[!]", "یعنی از این آدرس هیچ لاگین خودکاری جواب نمی‌ده -")
            _say("[!]", "نه موبایل، نه وب. هر دو چک‌پوینت صفحه‌ای می‌دن که")
            _say("[!]", "مسیر کد نداره.")
            _say("[!]", "")
            _say("[!]", "چیزی که کار می‌کنه: تو مرورگر خودت لاگین کن - اونجا کد")
            _say("[!]", "میاد چون آدرس تو مشکلی نداره - بعد کوکیشو بیار:")
            _say("[!]", "    botctl igdirect  ←  گزینه ۲  ←  گزینه ۱")
        return False
